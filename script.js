document.addEventListener('DOMContentLoaded', async () => {
  ...
  function updateAdminBadge(name, dbIsAdmin=false, dbIsChief=false){
    ...
    if(adminBadge){ adminBadge.hidden=!isAdmin; adminBadge.textContent=(Boolean(dbIsChief)||String(name||'').trim().toLowerCase()==='administrator')?'CHIEF ADMIN':'ADMIN'; }
    if(isAdmin){
      if(!topAdminBadge){
        ...
        topAdminBadge.textContent=dbIsChief||String(name||'').trim().toLowerCase()==='administrator'?'CHIEF ADMIN':'ADMIN';
    ...
    if(topAdminBadge && isAdmin) topAdminBadge.textContent=(Boolean(dbIsChief)||String(name||'').trim().toLowerCase()==='administrator')?'CHIEF ADMIN':'ADMIN';
  }
  ...
  const userSearch=document.getElementById('userSearch'), userSearchBtn=document.getElementById('userSearchBtn'), userResults=document.getElementById('userResults'), adminsList=document.getElementById('adminsList');
  ...
  async function searchUsers(){
    ...
    const {data,error}=await supabase.from('profiles').select('id,username,display_name,avatar_url,created_at,is_admin,is_chief_admin').ilike('username','%'+q+'%').order('username').limit(30);
    ...
    userResults.innerHTML=data.map(u=>{
      ...
      return `<div class="user-card" data-user-id="${escapeHtml(u.id)}"><img src="${escapeHtml(avatar)}" alt=""><div class="user-card-info"><strong>${escapeHtml(name)}</strong><span>@${escapeHtml(u.username||'')}</span></div>${isAdmin?`<span class="admin-badge user-card-admin">${isChief?'CHIEF ADMIN':'ADMIN'}</span>`:''}<button class="download secondary small-btn profile-open-btn" type="button" data-user-profile="${escapeHtml(u.id)}">ПРОФИЛЬ</button></div>`;
    }).join('');
  }
  async function loadAdmins(){
    ...
    adminsList.innerHTML=admins.map(u=>{
      ...
      const isChief=Boolean(u.is_chief_admin)||String(u.username||'').trim().toLowerCase()==='administrator';
      const avatar=u.avatar_url||avatarFallback(name);
      return `<div class="user-card" data-user-id="${escapeHtml(u.id)}"><img src="${escapeHtml(avatar)}" alt=""><div class="user-card-info"><strong>${escapeHtml(name)}</strong><span>@${escapeHtml(u.username||'')}</span></div><span class="admin-badge user-card-admin">${isChief?'CHIEF ADMIN':'ADMIN'}</span><button class="download secondary small-btn profile-open-btn" type="button" data-user-profile="${escapeHtml(u.id)}">ПРОФИЛЬ</button></div>`;
    }).join('');
  }
  if(adminsList){
