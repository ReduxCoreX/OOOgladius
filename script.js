document.addEventListener('DOMContentLoaded', async () => {
  ...
  function updateAdminBadge(name, dbIsAdmin=false, dbIsChief=false){
    ...
    if(adminBadge){ adminBadge.hidden=!isAdmin; adminBadge.textContent=(Boolean(dbIsChief)||String(name||'').trim().toLowerCase()==='administrator')?'CREATOR':'ADMIN'; }
    if(isAdmin){
      if(!topAdminBadge){
        ...
        topAdminBadge.textContent=dbIsChief||String(name||'').trim().toLowerCase()==='administrator'?'CREATOR':'ADMIN';
    ...
    if(topAdminBadge && isAdmin) topAdminBadge.textContent=(Boolean(dbIsChief)||String(name||'').trim().toLowerCase()==='administrator')?'CREATOR':'ADMIN';
  }
