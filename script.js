document.addEventListener('DOMContentLoaded', async () => {
  ...
  const dimToggle=document.getElementById('settingDim');
  const savedAnim=localStorage.getItem('rcx_animations');
  const savedDim=localStorage.getItem('rcx_dim');
  if(animToggle && savedAnim!==null) animToggle.checked=savedAnim==='1';
  if(dimToggle && savedDim!==null) dimToggle.checked=savedDim==='1';
  function applySettings(){
    document.body.classList.toggle('no-animations', animToggle ? !animToggle.checked : false);
    document.body.classList.toggle('low-dim', dimToggle ? !dimToggle.checked : false);
  }
  animToggle?.addEventListener('change',()=>{localStorage.setItem('rcx_animations',animToggle.checked?'1':'0');applySettings();});
  dimToggle?.addEventListener('change',()=>{localStorage.setItem('rcx_dim',dimToggle.checked?'1':'0');applySettings();});
  applySettings();
  ...
  let refreshUserSeq=0;
  async function refreshUser(user){
    const seq=++refreshUserSeq;
    try{
      if(!user){
    ...
    if(!user){
      ...
      accountText.textContent='ВОЙТИ'; topAvatar.hidden=true;
      if(homeWelcomeName)homeWelcomeName.textContent='участник';
      return;
    }
    try{
      const profile=await ensureProfile(user);
      if(seq!==refreshUserSeq)return;
      const name=profile.display_name || profile.username || user.user_metadata?.display_name || user.user_metadata?.username || 'User';
      ...
      if(homeWelcomeName)homeWelcomeName.textContent=name;
      profileName.textContent=name; profileDisplayName.textContent=name; profileUsername.textContent=profile.username || '';
      profileUsername.classList.add('private-value'); profileUsername.classList.remove('private-visible');
      profileEmail.textContent=user.email || ''; profileEmail.classList.add('email-blurred'); profileEmail.classList.remove('email-visible');
      emailToggle.textContent='ПОКАЗАТЬ'; usernameToggle.textContent='ПОКАЗАТЬ';
      updateAdminBadge(profile.username,profile.is_admin,profile.is_chief_admin);
      const avatar=profile.avatar_url || user.user_metadata?.avatar_url || avatarFallback(name);
      profileAvatar.src=avatar; topAvatar.src=avatar; topAvatar.hidden=false;
      ...
      loggedOut.hidden=true; loggedIn.hidden=false;
      loggedOut.style.display='none'; loggedIn.style.display='block';
      accountText.textContent=user.user_metadata?.display_name||user.email?.split('@')[0]||'АККАУНТ';
    }
  }
  ...
  function updateAdminBadge(name, dbIsAdmin=false, dbIsChief=false){
    const isAdmin=Boolean(dbIsAdmin)||Boolean(dbIsChief)||String(name||'').trim().toLowerCase()==='administrator';
    if(adminBadge){
      adminBadge.hidden=!isAdmin;
      adminBadge.textContent=(Boolean(dbIsChief)||String(name||'').trim().toLowerCase()==='administrator')?'CREATOR':'ADMIN';
    }
    if(topAdminBadge)topAdminBadge.hidden=true;
  }
  ...
  const userSearch=document.getElementById('userSearch'), userSearchBtn=document.getElementById('userSearchBtn'), userResults=document.getElementById('userResults'), onlineUsersCount=document.getElementById('onlineUsersCount'), adminsList=document.getElementById('adminsList');
  ...
  const publicAdminActions=document.getElementById('publicAdminActions'), toggleUserAdmin=document.getElementById('toggleUserAdmin'), adminActionStatus=document.getElementById('adminActionStatus');
  ...
  function getOnlinePresenceIds(){
    ...
    return [...new Set(Object.values(state).flat().map(meta=>meta?.user_id).filter(Boolean))];
  }

  async function loadOnlineUsers(query=''){
    if(!userResults)return;
    const ids=getOnlinePresenceIds();
    if(!ids.length){
      if(onlineUsersCount)onlineUsersCount.textContent='0 ОНЛАЙН';
      userResults.innerHTML='<div class="empty-history">Сейчас на сайте нет участников онлайн.</div>';
      return;
    }
    const {data,error}=await supabase.from('profiles').select('id,username,display_name,avatar_url,created_at,is_admin,is_chief_admin').in('id',ids).order('username');
    if(error){userResults.innerHTML='<div class="empty-history">Не удалось загрузить участников онлайн.</div>';return;}
    let online=data||[];
    const q=String(query||'').trim().toLowerCase();
    if(q)online=online.filter(u=>String(u.username||'').toLowerCase().includes(q)||String(u.display_name||'').toLowerCase().includes(q));
    if(onlineUsersCount)onlineUsersCount.textContent=`${online.length} ОНЛАЙН`;
    if(!online.length){userResults.innerHTML='<div class="empty-history">По этому запросу онлайн-участники не найдены.</div>';return;}
    userResults.innerHTML=online.map(u=>{
      const name=u.display_name||u.username||'User';
      const isAdmin=Boolean(u.is_admin)||String(u.username||'').trim().toLowerCase()==='administrator';
      const isChief=Boolean(u.is_chief_admin)||String(u.username||'').trim().toLowerCase()==='administrator';
      const avatar=u.avatar_url||avatarFallback(name);
      return `<div class="user-card" data-user-id="${escapeHtml(u.id)}"><img src="${escapeHtml(avatar)}" alt=""><div class="user-card-info"><strong>${escapeHtml(name)}</strong><span>@${escapeHtml(u.username||'')}</span></div>${isAdmin?`<span class="admin-badge user-card-admin">${isChief?'CREATOR':'ADMIN'}</span>`:''}<button class="download secondary small-btn profile-open-btn" type="button" data-user-profile="${escapeHtml(u.id)}">ПРОФИЛЬ</button></div>`;
    }).join('');
  }

  async function restartOnlinePresence(user){
    onlinePresenceUserId=user?.id||null;
    if(onlinePresenceChannel){
      try{await supabase.removeChannel(onlinePresenceChannel);}catch(e){}
      onlinePresenceChannel=null;
    }
    onlinePresenceKey=onlinePresenceUserId || (crypto.randomUUID ? crypto.randomUUID() : String(Date.now())+Math.random());
    onlinePresenceChannel=supabase.channel('rcx-online',{config:{presence:{key:onlinePresenceKey}}});
    onlinePresenceChannel
      .on('presence',{event:'sync'},()=>loadOnlineUsers(userSearch?.value||''))
      .on('presence',{event:'join'},()=>loadOnlineUsers(userSearch?.value||''))
      .on('presence',{event:'leave'},()=>loadOnlineUsers(userSearch?.value||''));
    await onlinePresenceChannel.subscribe(async status=>{
      if(status==='SUBSCRIBED' && onlinePresenceUserId){
        try{await onlinePresenceChannel.track({user_id:onlinePresenceUserId});}catch(e){console.warn('Presence error:',e.message);}
      }
      if(status==='SUBSCRIBED')await loadOnlineUsers(userSearch?.value||'');
    });
  }

  async function searchUsers(){
    await loadOnlineUsers(userSearch?.value||'');
  }

  async function loadAdmins(){
    if(!adminsList)return;
    adminsList.innerHTML='<div class="empty-history">Загрузка администраторов...</div>';
    const {data,error}=await supabase.from('profiles').select('id,username,display_name,avatar_url,created_at,is_admin,is_chief_admin').eq('is_admin',true).order('username');
    if(error){adminsList.innerHTML='<div class="empty-history">Не удалось загрузить список администраторов.</div>';return;}
    const admins=(data||[]).filter(u=>Boolean(u.is_admin)||String(u.username||'').trim().toLowerCase()==='administrator');
    if(!admins.length){adminsList.innerHTML='<div class="empty-history">Сейчас администраторов нет.</div>';return;}
    adminsList.innerHTML=admins.map(u=>{
      const name=u.display_name||u.username||'User';
      const isChief=Boolean(u.is_chief_admin)||String(u.username||'').trim().toLowerCase()==='administrator';
      const avatar=u.avatar_url||avatarFallback(name);
      return `<div class="user-card" data-user-id="${escapeHtml(u.id)}"><img src="${escapeHtml(avatar)}" alt=""><div class="user-card-info"><strong>${escapeHtml(name)}</strong><span>@${escapeHtml(u.username||'')}</span></div><span class="admin-badge user-card-admin">${isChief?'CREATOR':'ADMIN'}</span><button class="download secondary small-btn profile-open-btn" type="button" data-user-profile="${escapeHtml(u.id)}">ПРОФИЛЬ</button></div>`;
    }).join('');
  }
  if(adminsList){
  ...
  userSearch.addEventListener('input',()=>loadOnlineUsers(userSearch.value));
  userSearch.addEventListener('keydown',e=>{if(e.key==='Enter')searchUsers();});
  document.querySelectorAll('[data-open=\"users\"]').forEach(el=>el.addEventListener('click',()=>setTimeout(()=>loadOnlineUsers(userSearch.value),0)));

  let publicProfileUserId=null;
  async function openPublicProfile(id){
    const {data,error}=await supabase.from('profiles').select('id,username,display_name,avatar_url,created_at,is_admin,is_chief_admin').eq('id',id).maybeSingle();
    ...
    const targetAdmin=Boolean(data.is_admin)||String(data.username||'').trim().toLowerCase()==='administrator';
    const {data:{user:viewer}}=await supabase.auth.getUser();
    const viewerProfile=viewer?await ensureProfile(viewer):null;
    const viewerIsAdministrator=Boolean(viewerProfile&&String(viewerProfile.username||'').trim().toLowerCase()==='administrator');
    publicAdminBadge.hidden=!targetAdmin;
    const {user,profile}=await getCurrentProfile();
    const canManage=Boolean(user&&viewerIsAdministrator&&String(profile?.id||'')!==String(id));
    publicAdminActions.hidden=!canManage;
    adminActionStatus.textContent='';
    if(canManage){
      toggleUserAdmin.textContent=targetAdmin?'ЗАБРАТЬ ADMIN':'ВЫДАТЬ ADMIN';
    toggleUserAdmin.disabled=!viewerIsAdministrator;
      toggleUserAdmin.dataset.targetAdmin=targetAdmin?'true':'false';
    }
  ...
  toggleUserAdmin.onclick=async()=>{
    ...
    const {user,profile}=await getCurrentProfile();
    const isAdministrator=String(profile?.username||'').trim().toLowerCase()==='administrator';
    if(!user||!isAdministrator)return;
    const targetAdmin=toggleUserAdmin.dataset.targetAdmin==='true';
    toggleUserAdmin.disabled=true; adminActionStatus.textContent='Обновление...';
    const {error}=await supabase.rpc('set_user_admin',{target_user_id:publicProfileUserId,make_admin:!targetAdmin});
    toggleUserAdmin.disabled=false;
    if(error){adminActionStatus.textContent='Ошибка: '+(error.message||'');return;}
    adminActionStatus.textContent=!targetAdmin?'Админ выдан.':'Админ забран.';
    await openPublicProfile(publicProfileUserId);
    await searchUsers();
    await loadAdmins();
  };
  ...
  async function getCurrentProfile(){
    let user=null;
    try{
      const direct=await supabase.auth.getUser();
    ...
    if(!user)return {user:null,profile:null,isAdmin:false,isChief:false};
    const profile=await ensureProfile(user);
    return {user,profile,isAdmin:Boolean(profile.is_admin)||String(profile.username||'').trim().toLowerCase()==='administrator',isChief:Boolean(profile.is_chief_admin)||String(profile.username||'').trim().toLowerCase()==='administrator'};
  }
  ...
  async function loadReports(){
    ...
    reportsList.innerHTML=data.map(r=>{
      ...
      const accepted=(r.status==='in_progress'||r.status==='closed') && r.admin_id;
  ...
  async function openReportChat(reportId){
    ...
    if(reportError||!report||!['in_progress','closed'].includes(report.status)||!report.admin_id||(report.user_id!==user.id&&report.admin_id!==user.id)){
      alert('Чат недоступен.');
      return;
    }
    activeChatReportId=reportId;
    chatReportInfo.textContent='@'+(report.reporter_username||'unknown')+' • '+(report.admin_username?'@'+report.admin_username:'администратор')+(report.status==='closed'?' • CLOSED':'');
    chatInput.disabled=report.status==='closed'; sendChatMessage.disabled=report.status==='closed';
  ...
  async function loadChatMessages(){
    ...
    const {data:report}=await supabase.from('reports').select('message,status,reporter_username,created_at').eq('id',activeChatReportId).maybeSingle();
    const {data:{user}}=await supabase.auth.getUser();
    const original=report?`<div class="chat-message theirs report-original"><div class="chat-message-head"><strong>@${escapeHtml(report.reporter_username||'user')} • ОБРАЩЕНИЕ</strong><time>${new Date(report.created_at).toLocaleString('ru-RU')}</time></div><div class="chat-message-body">${escapeHtml(report.message||'')}</div></div>`:'';
    ...
    chatMessages.innerHTML=original+(history||'<div class="empty-history">История сообщений пока пуста.</div>');
    if(report?.status==='closed'){chatMessageStatus.textContent='Обращение закрыто. История доступна только для просмотра.';}
    chatMessages.scrollTop=chatMessages.scrollHeight;
  }
  function closeReportChatModal(){
    clearInterval(chatPollTimer); chatPollTimer=null; activeChatReportId=null;
    reportChatModal.classList.remove('open'); reportChatModal.setAttribute('aria-hidden','true'); chatInput.disabled=false; sendChatMessage.disabled=false; chatInput.value=''; chatMessageStatus.textContent='';
  }
  ...
  supabase.auth.onAuthStateChange((_event,session)=>{
    setTimeout(async()=>{
      ...
      if(!activeUser){try{activeUser=(await supabase.auth.getUser())?.data?.user||null;}catch(e){}}
      await refreshUser(activeUser);
      try{await restartOnlinePresence(activeUser);}catch(e){console.warn('Presence error:',e.message||e);}
      if(document.getElementById('users')?.classList.contains('active')) await loadOnlineUsers(userSearch?.value||'');
      if(document.getElementById('reports')?.classList.contains('active')) await loadReports();
    },50);
  });
  let initialUser=null;
  try{initialUser=(await supabase.auth.getUser())?.data?.user||null;}catch(e){}
  if(!initialUser){try{initialUser=(await supabase.auth.getSession())?.data?.session?.user||null;}catch(e){}}
  await refreshUser(initialUser);
  try{await restartOnlinePresence(initialUser);}catch(e){console.warn('Presence init error:',e.message||e);}
  await loadOnlineUsers('');
  if(document.getElementById('reports')?.classList.contains('active')) await loadReports();

  // Subtle GTA-style UI sounds: hover + click, generated locally with Web Audio.
  let audioCtx=null;
  function uiSound(kind='hover'){
    try{
      ...
      osc.frequency.setValueAtTime(kind==='click'?420:260,now);
      osc.frequency.exponentialRampToValueAtTime(kind==='click'?620:330,now+.055);
      gain.gain.setValueAtTime(0.0001,now);
      gain.gain.exponentialRampToValueAtTime(kind==='click'?0.035:0.014,now+.008);
      gain.gain.exponentialRampToValueAtTime(0.0001,now+.085);
      osc.connect(gain); gain.connect(audioCtx.destination);
      osc.start(now); osc.stop(now+.09);
    }catch(e){}
  }
  document.querySelectorAll('button,a,.feature-card,.account-chip,.setting-row').forEach(el=>{
    el.addEventListener('mouseenter',()=>uiSound('hover'),{passive:true});
    el.addEventListener('click',()=>uiSound('click'),{passive:true});
  });
});
