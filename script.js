document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function openPublicProfile(id){
    ...
    const viewerIsChief=Boolean(viewerProfile?.is_chief_admin)||String(viewerProfile?.username||'').trim().toLowerCase()==='administrator';
    publicAdminBadge.hidden=!targetAdmin;
    const {user,profile,isAdmin}=await getCurrentProfile();
    const canManage=Boolean(user&&viewerIsChief&&String(profile?.id||'')!==String(id));
    ...
    toggleUserAdmin.disabled=!viewerIsChief;
  ...
  toggleUserAdmin.onclick=async()=>{
    ...
    const {user,profile,isAdmin,isChief}=await getCurrentProfile();
    if(!user||!isAdmin||!isChief)return;
